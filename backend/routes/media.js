...
export default class MediaRoutes {
    ...
    setup() {
        this.router.post(
            '/',
            authMiddleware, 
            this.SchemaValidator.validate(MediaSchema.create), 
            this.mediaController.create
        );

        this.router.get(
            '/:id', 
            this.SchemaValidator.validate(MediaSchema.getOne), 
            this.mediaController.findOne
        );

        this.router.get(
            '/user/:userId', 
            this.mediaController.findAllByUser
        );
