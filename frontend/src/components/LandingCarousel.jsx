...
const CustomLeftArrow = ({ onClick }) => (
	<button
		onClick={onClick}
		className="ag-absolute ag-left-0 ag-top-1/2 -ag-translate-y-1/2 ag-text-yellowag ag-text-6xl hover:ag-text-opacity-50 transition md:ag-flex md:ag-text-8xl">
		‹
	</button>
)
...
const CustomRightArrow = ({ onClick }) => (
	<button
		onClick={onClick}
		className="ag-absolute ag-right-0 ag-top-1/2 -ag-translate-y-1/2 ag-text-yellowag ag-text-6xl hover:ag-text-opacity-50 transition md:ag-flex md:ag-text-8xl">
		›
	</button>
)
...
const CustomDot = ({ index, onClick, active, carouselState }) => {
	...
	return (
		<li className={`ag-w-4 ag-h-4 ag-rounded-full ag-mx-1 ${dotClass}`}/>
	)
}
